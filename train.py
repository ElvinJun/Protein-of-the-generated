...
if 1:
    ...
    # Dataset.
    desc += '-custom-dataset';      dataset = EasyDict(tfrecord_dir='custom-dataset');                 train.mirror_augment = True
...
def main():
    kwargs = EasyDict(train)#对应submit_run形参中的run_func_name
    kwargs.update(G_args=G, D_args=D, G_opt_args=G_opt, D_opt_args=D_opt, G_loss_args=G_loss, D_loss_args=D_loss)
    kwargs.update(dataset_args=dataset, sched_args=sched, grid_args=grid, metric_arg_list=metrics, tf_config=tf_config)#对应submit_run中的**run_func_kwargs
